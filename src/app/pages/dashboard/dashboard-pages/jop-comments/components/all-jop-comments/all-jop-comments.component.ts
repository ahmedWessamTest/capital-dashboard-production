...
import { JobRequestResponse } from '../../../jop-requests/services/jop-requests.service';
import {
  CreateJobCommentRequest,
  JobRequestComment,
  JobRequestCommentsService,
} from '../../services/jop-comments.service';

@Component({
  selector: 'app-job-request-comments',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './all-jop-comments.component.html',
  styleUrl: './all-jop-comments.component.scss',
  providers: [MessageService],
})
export class AllJopCommentsComponent implements OnInit, OnDestroy {
  ...
  requestData!: JobRequestResponse;
  ...
  sendComment() {
    ...
    const commentData: CreateJobCommentRequest = {
      ...
      reciver_name: this.requestData.data.name || 'Receiver name',
      request_id: Number(this.requestData.data.id),
      request_status: this.requestData.data.active_status,
    };

    this.commentsService
      .create(this.requestId, commentData, this.selectedFile || undefined)
      .subscribe({
        next: (response) => {
