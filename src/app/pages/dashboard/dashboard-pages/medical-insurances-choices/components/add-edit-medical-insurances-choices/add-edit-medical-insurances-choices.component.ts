...
@Component({
  selector: 'app-add-edit-medical-insurances-choices',
  standalone: true,
  imports: [CommonModule, GenericFormComponent],
  templateUrl: './add-edit-medical-insurances-choices.component.html',
  styleUrl: './add-edit-medical-insurances-choices.component.scss'
})
export class AddEditMedicalChoiceComponent implements OnInit {
  formFields: any[] = [];
  editData: any = null;
  extraFormData: { [key: string]: string | number } = {};
  isEditMode = false;

  medicalInsuranceId: string | null = null;
  ...
  ngOnInit(): void {
    this.medicalInsuranceId = this.route.snapshot.paramMap.get('id');
