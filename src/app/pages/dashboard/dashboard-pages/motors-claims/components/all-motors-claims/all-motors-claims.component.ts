...
import { MotorClaimsService, MotorClaim } from '../../../../../../core/services/claims/motor-claim.service';
...
export class AllMotorsClaimsComponent implements AfterViewInit {
  ...
  getClaimType(claim: MotorClaim): string {
    return claim.motor_insurance_id ? 'Ordinary' : 'Manual';
  }
  ...
  compareValues(a: MotorClaim, b: MotorClaim, field: string | null, order: number): number {
    if (!field) return 0;

    let valueA: any = a[field as keyof MotorClaim];
    let valueB: any = b[field as keyof MotorClaim];
  ...
  updateClaimStatus(claim: MotorClaim, status: 'confirmed' | 'canceled' | 'pending' | 'requested') {
    this.ngxSpinnerService.show('actionsLoader');

    const formData = new FormData();
    formData.append('status', status);

    this.motorClaimsService.update(claim.id, formData).subscribe({
      next: (response) => {
