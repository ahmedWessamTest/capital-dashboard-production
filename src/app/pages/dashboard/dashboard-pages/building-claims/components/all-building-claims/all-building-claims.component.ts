...
import { BuildingClaimsService, BuildingClaim } from '../../../../../../core/services/claims/building-claim.service';
...
@Component({
  selector: 'app-all-building-claims',
  standalone: true,
  imports: [
    ButtonModule,
    DropdownModule,
  ...
  templateUrl: './all-building-claims.component.html',
...
export class AllBuildingClaimsComponent implements AfterViewInit {
  ...
  getClaimType(claim: BuildingClaim): string {
    return claim.building_insurance_id ? 'Ordinary' : 'Manual';
  }
    ...
    if (this.selectedType !== null) {
      filtered = filtered.filter((claim) =>
        this.selectedType === 'manual'
          ? claim.building_insurance_id === null
          : claim.building_insurance_id !== null
      );
    }
  ...
  compareValues(a: BuildingClaim, b: BuildingClaim, field: string | null, order: number): number {
    if (!field) return 0;

    let valueA: any = a[field as keyof BuildingClaim];
    let valueB: any = b[field as keyof BuildingClaim];
  ...
  updateClaimStatus(claim: BuildingClaim, status: 'confirmed' | 'canceled' | 'pending' | 'requested') {
    this.ngxSpinnerService.show('actionsLoader');

    const formData = new FormData();
    formData.append('status', status);

    this.buildingClaimsService.update(claim.id, formData).subscribe({
      next: (response) => {
