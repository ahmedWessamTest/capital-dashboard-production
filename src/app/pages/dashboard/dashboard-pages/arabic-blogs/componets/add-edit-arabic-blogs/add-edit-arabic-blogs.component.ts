...
import { ArabicBlogsService, ArabicBlogResponse, ArabicBlogFormData } from '../../service/arabic-blogs.service';
...
@Component({
  selector: 'app-add-edit-arabic-blogs',
  standalone: true,
  imports: [
    ReactiveFormsModule,
    ButtonModule,
  ...
  templateUrl: './add-edit-arabic-blogs.component.html',
  styleUrl: './add-edit-arabic-blogs.component.scss',
  providers: []
})
export class AddEditArabicBlogsComponent implements OnInit {
  private router = inject(Router);
  private fb = inject(FormBuilder);
  private arabicBlogsService = inject(ArabicBlogsService);
  ...
  loadBlogData() {
    this.route.data.subscribe({
      next: (data) => {
        const response = data['data'] as ArabicBlogResponse;
        ...
        this.blogForm.patchValue({
          ...
          ar_first_script_text: blog.ar_first_script_text,
  ...
  onSubmit() {
    ...
    const formData: ArabicBlogFormData = this.blogForm.value;
    formData.active_status = this.blogForm.value.active_status ? '1' : '0';
    const apiCall = this.isEditMode && this.blogId
      ? this.arabicBlogsService.update(+this.blogId, formData)
      : this.arabicBlogsService.create(formData);
    ...
    apiCall.subscribe({
      next: (response) => {
        ...
        this.ngxSpinnerService.hide('actionsLoader');
        setTimeout(() => {
          this.router.navigate(['/dashboard/menu/ar-blogs']);
          this.message = { text: '', type: '' };
        }, 1000);
      },
      error: (error: any) => {
