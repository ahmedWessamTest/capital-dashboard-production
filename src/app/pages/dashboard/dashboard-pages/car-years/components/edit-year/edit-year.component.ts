...
@Component({
  ...
  styleUrls: ['./edit-year.component.scss']
})
export class EditYearComponent implements OnInit, OnDestroy {
  carYearForm: FormGroup;
  carModels: CarModel[] = [];
  private dataSubscription: Subscription | null = null;
  loading: boolean = false;
  message = { text: '', type: '' };
  carYearId: number | null = null;
    ...
    this.dataSubscription = this.route.data.subscribe({
      next: (data: any) => {
        ...
        if (data?.data?.carYear && data?.data?.carModels) {
          ...
          this.carYearForm.patchValue({
            car_model_id: data.data.carYear.car_model_id,
            year_date: data.data.carYear.year_date,
            active_status: this.normalizeActiveStatus.normalizeActiveStatus(data.data.carYear.active_status)
          });
          this.ngxSpinnerService.hide('actionsLoader');
        } else {
          this.message = { text: 'Failed to load car year or models data', type: 'error' };
          this.clearMessage();
          this.ngxSpinnerService.hide('actionsLoader');
        }
      },
      error: (err) => {
        console.error('Failed to load car year data:', err);
  ...
  onSubmit() {
    ...
    if (!this.carYearId) {
      this.message = { text: 'No car year ID provided', type: 'error' };
      this.clearMessage();
      return;
    }
    ...
    this.carYearsService.getAll().subscribe({
      next: (res) => {
        const yearsForSelectedModel = res.data
          .filter((item: CarYear) => item.car_model_id === selectedCarModelId && item.id !== this.carYearId)
